...
module.exports=async({getNamedAccounts,deployments})=>{
    ...
    const testing=await deploy("testing",{
        from:deployer,
        ...
        proxy:{
            ...
            viaAdminContract:{
                name:"testingProxyAdmin",
                artifact:"testingProxyAdmin",
            },   
        },
        waitConfirmation:1
    })
    log("contract_address=",testing.address);
    if (!developmentChains.includes(network.name) && process.env.ETHERSCAN_API_KEY) {
        log("Verifying...")
        await verify(testing.address, arguments)
    }
